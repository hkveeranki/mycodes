...
inline ll get_dist(lpair a,lpair b){ return ((a.F-b.F)*(a.F-b.F) + (a.S-b.S)*(a.S-b.S));}
...
int main(){
	...
	TC(){
		int n;
		cin >> n;
		rep(i,1,n) cin >> vert[i].F >> vert[i].S; 
		ll ans = 0;
		rep(i,1,n){
			dist.clear();
			rep(j,1,n){
				if(i!=j){
					ll val = get_dist(vert[i],vert[j]);
			//		out3(i,j,val);
					dist.PB(val);
				}
			}
			sort(ALL(dist));
			//out1(dist.size());
			ll cnt = 1,prev = dist[0];
			repl(j,1,dist.size()){
				//out4(j,dist[j],prev,cnt);
				if(dist[j] == prev)cnt++;
				else{
					ans+=(cnt*(cnt-1))/2;
					cnt = 1;
