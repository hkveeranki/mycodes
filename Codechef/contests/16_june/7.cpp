		...
		TC(){
			...
			if(q == 1){
				...
				if(pos == 1) fir = val;
					...
					ll inv_val = inv(a[pos],MOD)%MOD;
					double diff = log10(val)-log10(a[pos]);
					ll dif = (inv_val*val)%MOD;
					ll i;
					for(i=1;i*i<pos-1;i++){
						if((pos-1)%i == 0){
							ll ind = (pos-1)/i;
							mul[i] = (mul[i]*dif)%MOD;
							logs[i] = logs[i]+diff;
							mul[ind] = (mul[ind]*dif)%MOD;
							logs[ind] = logs[ind]+diff;
						}
					}
					if(i*i == (pos-1) && (pos-1)%i == 0){
						mul[i] = (mul[i]*dif)%MOD;
						logs[i] = logs[i]+diff;
					}
					a[pos] = val;
				...
				cin >> pos;
				double tmp = logs[pos]+log10(fir);
				ll firs = pow(10,tmp-(int)tmp);
				ll ans = (mul[pos]*fir)%MOD;
				cout << firs << " " << ans << endl;
			}
		}
	}
