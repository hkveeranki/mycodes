...
inline void Fact(){fact[0]=1;for(ll i=1;i<LIM2;i++)fact[i]=(i*fact[i-1])%MOD;}
...
int main(){
	...
	TC(){
		...
		if(cnt == n) cout << "1\n";
			...
			ll ans = 0;
			if(cnt == 0)
				for(int j = k%2;j<=n && j<=k;j+=2) ans=(ans+comb(n,j))%MOD;
			else
				for(int j=0;j<=n && j<=k;j++) ans=(ans+comb(n,j))%MOD;
			cout << ans << endl;
		}
	}
	//exectime();
