...
int main(){
	...
	char digits[] = {'0','2','4','6','8'};
	powers[0]=1;
	rep(i,1,25)powers[i]=powers[i-1]*5;
	TC(){
		ll k,sum=5;
		int dig;
		cin >> k;
		if(k<6){
			...
			// Filling first digit
			rep(i,1,4){
				//out2(k,i*powers[dig-1]);
				if(k< i* powers[dig-1]){
					ans[0] = digits[i];
					k-=(i-1)*powers[dig-1];
					break;
				}
				else if(k == i*powers[dig-1]){
					ans[0] = digits[i];
					rep(j,1,dig-1)ans[j] = digits[4];
					fl = 0;
					break;
				}
			}
			...
			rep(j,2,dig){
				if(fl == 0) break;
					rep(i,0,4){
						//out2(k,powers[dig-j]*(i+1));
						if((i+1)*powers[dig-j]>k){
							ans[j-1] = digits[i];
							k-=(i)*powers[dig-j];
							break;
						}
						else if((i+1)*powers[dig-j] == k){
							ans[j-1]=digits[i];
							rep(l,j,dig-1)ans[l] = '8';
							fl = 0;
							break;
						}
