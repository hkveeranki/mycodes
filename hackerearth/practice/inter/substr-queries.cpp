...
bool check(){
	int i;
	rep(i,0,255)if(cnt[i]<cnts[i])return false;
	return true;
}

ll solve(){
	int i,j,n,n1=s.length();
	n=qstr.length();
	init(cnts,0);
	init(cnt,0);
	repl(i,0,n)cnts[qstr[i]]++;
	ll left=0,right=0,ans=0;
	while(right<n1 && !check()){
		cnt[s[right]]++;
		right++;
	}
	if(!check())return 0;
	...
	repl(left,1,n1){
		cnt[s[left-1]]--;
		while(right<n1 && cnt[s[left-1]]<cnts[s[left-1]]){cnt[s[right]]++;right++;}
		if(cnt[s[left-1]]<cnts[s[left-1]])break;
		ans+=(n1-right+1);
	}
	return ans;
}
