...
void uni(int x,int y){
	...
	px=find(x);
	py=find(y);
	if(myrank[px]<myrank[py]) parent[px]=py;
	else if(myrank[py]<myrank[px])parent[py]=px;
	else{
		parent[px]=py;
		myrank[py]++;
	}
...
int main(){
	...
	repi(i,m,1){
		if(find(a[i].F) == find(a[i].S) )
			ans.PB(i);
		else uni(a[i].F,a[i].S);
	}
	reverse(ALL(ans));
	cout << ans.size() << endl;
	repl(i,0,(int)ans.size()) cout << ans[i] << endl; 
	//exectime();
	return 0;
}
