...
void add(int node,int index){
	//if(index==len)return ;
	if(index==len){
		a[node]++;
		return ;
	}
	add(2*node+s[index],index+1);
}
void del(int node , int index){
	if(index==len){
		a[node]--;
		return ;
	}
	del(2*node+s[index],index+1);
}
int cnt(int node,int index){
	if(index==len)return a[node];
	int val=cnt(2*node+s[index],index+1);
	if(s[index]==1)val+=cnt(2*node,index+1);
	return a[node]+val;
}
/*main code*/
...
int main(){
	...
	TC(){
		...
		f(n);
		if(q=="add")add(1,0);
		else if(q=="del")del(1,0);
		else cout << cnt(1,0) << endl;
	}
	//exectime();
	return 0;
}
