...
int main(){
	...
	string s;
	int i,ans=0,n,minm,maxm;
	cin >> s;
	n = s.length();
	repl(i,0,n)cnts[s[i]-'a']++;
	i=0;
	minm = n+1;
	maxm = 0;
	while(i<26){
		if(cnts[i]!=0){
			minm = min(cnts[i],minm);
			maxm = max(cnts[i],maxm);
			m[cnts[i]]++;
			ans++;
		}
		i++;
	}
	if(maxm == minm ||(minm == 1 && m[maxm]+m[minm] == ans)||(maxm - minm == 1 && (m[minm] == 1 || m[maxm] == 1)) ) cout << "YES\n";
	else cout << "NO\n";
