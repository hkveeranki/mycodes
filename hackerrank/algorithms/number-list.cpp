...
int main(){
	...
	TC(){
		...
		while(i <= n){
			...
			while(a[j] <= k && j <= n) j++;
			if(j > n){
				// No element greater than k found so no more sub arrays
				break;
			}
			// now any sub array which has start between i to j and end from j to n will have a maximum 
			// which is greater than k since the current element is greater than k so add that much to the answer
			ans += (j-i+1)*(n - j + 1);
			i = j+1;
		}
		cout << ans << endl;
