...
int main(){
	...
	rep(i,1,k){
		...
		else if(abs(x - qx) == abs(y - qy)){
			if(x < qx){
				if(y < qy) 
				m[4] = min(m[4], min(qx - x, qy - y) - 1);
				else 
				m[5] = min(m[5], min(qx - x, y - qy) - 1);
			}
			else{
				if(y < qy) 
				m[6] = min(m[6], min(x - qx, qy - y) - 1);
				else 
				m[7] = min(m[7], min(x - qx, y - qy) - 1);
			}
		}
	}
	//rep(i,0,7) cout << m [i] << " "; cout << endl;
