...
int main(){
	...
	priority_queue<ll, vector<ll>, less<ll> > row_sums;
	priority_queue<ll, vector<ll>, less<ll> > col_sums;
	ll n, m, k, p, x, ans = -99999999999999;
	cin >> n >> m >> k >> p;
	rep(i,1,n)
		rep(j,1,m){
			cin >> x;
	...
	rep(i,1,n) row_sums.push(r[i]);
	rep(j,1,m) col_sums.push(c[j]);
	rep(i,1,k){
		ll row_max = row_sums.top();
		ll col_max = col_sums.top();
		hr[i] = hr[i-1] + row_max;
		hc[i] = hc[i-1] + col_max;
		row_sums.pop();
		col_sums.pop();
		row_sums.push(row_max - m * p);
		col_sums.push(col_max - n * p);
	}
	rep(i,0,k){
		ans = max(ans, hr[i]+hc[k-i] - i * (k - i) * p);
	}
	cout << ans << endl;
	return 0;	
}
