...
/*Functions */
//Two main things u have to understand while doing this problem are 1.There is only one cell  in diagonal given a row number or column number
//2. In L type query row doesnt change and U type query column doesnt change. in L type   query the eatable part of  given row gets eated away. and in U query eatable part of that column given gets eated way.
//Here i mean when we see according to rows see only according to  rows. and vice versa.
//Hope u can understand the solution now. If you have  further queries contact me.

void ans(int x,int y,char q){
	map<int,pair<char,int> >::iterator it=m.lower_bound(x);
	if(it->F==x){
		cout<<0<<endl;//This is pretty clear.. Its already eaten;
		return ;
	}
	if(q=='L')it--;//Because we  are  going left and we need to know  the least possible column thats already eaten. so we do it--
	int  ans1=abs(it->F-x);//This is clear .
	if(it->S.F==q)ans1+=it->S.S;//This is because if we do U from x in column (say 4) and now if  we start at 3 now, we know that this will be in have just one more row (it->f-X) more than the 4th one's  result.. Think why ??.. Same can be applied to L because there is a relation between row and column. 
	cout<<ans1<<endl;
	m[x]=MP(q,ans1);
}
