...
int query(int i,int l,int r,int s,int e){
	...
	int m=(l+r)>>1;
	if(e<=m)return query(i<<1,l,m,s,e);
	if(s>m)return query(i<<1|1,m+1,r,s,e);
	return query(i<<1,l,m,s,m)&query(i<<1|1,m+1,r,m+1,e);
}

int inline f(int x,int b){return x&(1<<b);}
/*main code*/
...
int main(){
	...
	int i,b,n,m;
	cin>>n>>m;
	rep(i,1,m)cin>>inp[i].x>>inp[i].y>>inp[i].q;
	init(a,0);
	rep(b,0,30){
		rep(i,1,n)tmp[i]=0;
		rep(i,1,m){
		...
		rep(i,1,n) if(tmp[i])a[i]|=(1<<b);
	}
	build(1,1,n);
	bool fl=1;
	rep(i,1,m){
		if(query(1,1,n,inp[i].x,inp[i].y)!=inp[i].q){
			fl=0;
			break;
		}
	}
